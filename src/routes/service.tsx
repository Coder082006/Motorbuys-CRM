...
function Service() {
  ...
  const inProgress = records.filter(
    (r: any) => r.status === "in_progress" || r.status === "diagnosing",
  ).length;
  const completedToday = records.filter(
    (r: any) =>
      r.completed_date && new Date(r.completed_date).toDateString() === new Date().toDateString(),
  ).length;
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="grid gap-4 grid-cols-2 lg:grid-cols-4">
        ...
        <Stat label="Service Revenue" value={formatCurrency(revenue)} />
      </div>
      <Tabs defaultValue="srv">
        <TabsList>
          <TabsTrigger value="srv">Service Requests</TabsTrigger>
        ...
        <TabsContent value="srv">
          {recordsQ.isLoading ? (
            ...
            <Card className="p-6">
              <p>Failed to load service records</p>
              <div className="pt-2">
                <Button onClick={() => recordsQ.refetch()}>Retry</Button>
              </div>
            </Card>
          ) : (
            <Card>
                    ...
                    {records.map((s: any) => (
                      <TableRow key={s.id}>
                        ...
                        <TableCell className="text-muted-foreground">
                          {s.motorbike_name || s.motorbike}
                        </TableCell>
                        <TableCell>{s.service_type}</TableCell>
                        <TableCell>
                          <Badge className={stColor[s.status]}>{mapStatusLabel(s.status)}</Badge>
                        </TableCell>
                        ...
                        <TableCell className="text-right">
                          <div className="inline-flex gap-2">
                            <Select
                              value={s.status}
                              onValueChange={(v) =>
                                updateRecord.mutate({ id: s.id, data: { ...s, status: v } })
                              }
                            >
                              <SelectTrigger>
                                <SelectValue />
                              ...
                              <SelectContent>
                                <SelectItem value="received">Bike Received</SelectItem>
        ...
        <TabsContent value="parts">
          {partsQ.isLoading ? (
            ...
            <Card className="p-6">
              <p>Failed to load spare parts</p>
              <div className="pt-2">
                <Button onClick={() => partsQ.refetch()}>Retry</Button>
              </div>
            </Card>
          ) : (
            <Card>
                        ...
                        <TableCell className="text-right">
                          <div className="inline-flex gap-1">
                            ...
                            <Button
                              ...
                              onClick={() => {
                                if (confirm("Delete spare part?")) deletePart.mutate(p.id);
                              }}
                            >
                              Delete
                            </Button>
        ...
        <form className="grid grid-cols-2 gap-4">
          ...
          <div className="space-y-1.5">
            ...
            <Select>
              ...
              <SelectContent>
                <SelectItem value="eric">Eric W.</SelectItem>
                <SelectItem value="joel">Joel K.</SelectItem>
              </SelectContent>
            </Select>
          </div>
          <div className="space-y-1.5 col-span-2">
