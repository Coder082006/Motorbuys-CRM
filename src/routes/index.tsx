...
const modules = [
  { title: "Customers", description: "Manage customer profiles and leads", icon: Users, to: "/customers" },
  { title: "Inventory", description: "View and manage motorbike stock", icon: Bike, to: "/inventory" },
  { title: "Sales Pipeline", description: "Track deals from inquiry to sale", icon: TrendingUp, to: "/sales" },
  { title: "Financing", description: "Manage loans and installment payments", icon: CreditCard, to: "/financing" },
  { title: "Marketing & Advertising", description: "Campaigns, SMS and promotions", icon: Megaphone, to: "/marketing" },
  { title: "Service Center", description: "Manage bike maintenance requests", icon: Wrench, to: "/service" },
] as const;
              ...
              <div className="flex flex-col gap-4 px-4 py-4 sm:px-6 lg:flex-row lg:items-center lg:justify-between">
                <div className="flex items-center gap-3">
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <div className="text-lg font-semibold tracking-[0.12em] text-[#f97316]">MOTORBIKE CRM</div>
                ...
                <div className="flex flex-col items-start gap-2 text-left lg:items-center lg:text-center">
                  <div className="text-xl font-semibold text-white">Welcome back, {username}</div>
              ...
              <div className="mx-auto flex w-full max-w-7xl flex-col gap-6">
                <section className="grid gap-4 sm:grid-cols-2 xl:grid-cols-6">
                  {stats.map(({ label, value, icon: Icon }) => (
                    <div key={label} className="rounded-2xl border border-white/10 bg-[#1e293b] p-4 shadow-sm shadow-black/20">
                ...
                <section className="rounded-3xl border border-white/10 bg-[linear-gradient(135deg,rgba(30,41,59,0.95),rgba(15,23,42,0.98))] p-6 shadow-xl shadow-black/20">
                  <div className="mb-5 flex items-center justify-between gap-3">
                    <div>
                      ...
                      <p className="mt-1 text-sm text-slate-400">Jump into the modules your team uses every day.</p>
                    </div>
                      ...
                      <table className="min-w-full text-left text-sm">
                        ...
                        <tbody>
                          {followUps.map((row, index) => (
                            <tr key={`${row.customer}-${row.phone}`} className={index % 2 === 0 ? "bg-[#1e293b]" : "bg-[#233047]"}>
                      ...
                      <table className="min-w-full text-left text-sm">
                        ...
                        <tbody>
                          {recentSales.map((row, index) => (
                            <tr key={`${row.customer}-${row.date}`} className={index % 2 === 0 ? "bg-[#1e293b]" : "bg-[#233047]"}>
                              <td className="px-5 py-4 font-medium text-white">{row.customer}</td>
